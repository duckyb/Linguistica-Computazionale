...
class Corpus: # creo classi Corpus, per ciasuna delle quali faccio le dovute analisi
    def __init__ (self, path, name):
        ...
        self.pos_tags = [] # [ (part_of_speech, token), (...) ]
        ...
        self.top10trigrams = FreqDist(trigrams([b for (a, b) in self.pos_tags])).most_common(10)
        self.bigram_data = {
    ...
    def tokenized_text(self): # Tokenizzazione
        phrases = sent_tokenizer.tokenize(self.raw.lower())
        tokenList = [token for p in phrases for token in word_tokenize(p)]
        self.pos_tags = pos_tag(tokenList)
        return tokenList

    def top20_no_punct(self):
        punct = re.compile(r'[^\w]') # Lista di segni di punteggiatura
        return FreqDist([i for i in self.tokens if not punct.match(i)]).most_common(20)

    def top20_partsofspeech(self, t):
        return FreqDist([token for token, tag in self.pos_tags if tag.startswith(t)]).most_common(20)

    def t10t(self):
        tuples = FreqDist(self.pos_tags).most_common()
        return FreqDist([b for (a, b), c in tuples]).most_common(10)
    ...
    def bigram_prob(self):
        ...
        self.bigram_data['condit'].sort(key=lambda e : e[1], reverse=True) # ordinamento decrescente
        self.bigram_data['condit'] = self.bigram_data['condit'][:10] # stacco le 10 più frequenti
        # J O I N E D
        pos_bigrams = list(bigrams([p for t, p in self.pos_tags]))
        pos_bigrams_fdist = FreqDist(pos_bigrams)
        ...
        self.bigram_data['joined'].sort(key=lambda e : e[1], reverse=True)
        self.bigram_data['joined'] = self.bigram_data['joined'][:10]

    def namentity(self):
        # N A M E D   E N T I T Y
        tree = ne_chunk(pos_tag(word_tokenize(self.raw))) # ne_chunk necessita della suddivisione per frasi
        names = []
        for branch in tree:
            NE = ''
...
class Combine: # unisco determinati valori di due Corpus
    def __init__(self, foo, bar):
        ...
        self.tags    = foo.pos_tags + bar.pos_tags # pos_tag combinati
        ...
        self.fdistJJ = FreqDist(self.adj) # fdist aggettivi combinati
        ...
        self.fdistBI = FreqDist(self.small) # fdist bigrammi combinati                    
        ...
        for n in self.top20NN: # per ciascuno dei 20 sostantivi
            ...
            for b in self.small: # controllo ogni bigramma (aggettivo, sostantivo) del testo
                if b[1] == n[0] and b[0] in self.adj: # se il sostantivo è quello che cerco
                    ...
                    LMI = (freq_observed*1.0)*math.log((freq_observed*1.0)/(freq_expected*1.0), 2) # formula
                    JJeLMI_Tuples.append((b[0], LMI)) # genero una tupla JJ + LMI
            JJeLMI_Tuples.sort(key = lambda e : e[1], reverse = True) # ho finito; ordino le tuple x LMI
...
# ========================= MENU & TABELLE =========================
    ...
    while choice != 0:
        ...
        if choice == 1: # tabella senza punteggiatura
            ...
            mf = Combine(m, f) # creo una classe che combina i due corpus di partenza
